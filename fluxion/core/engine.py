...
class FluxionEngine:
    ...
    def _build_client(
        self, extra_headers: dict[str, str] | None = None, trace_http: bool = False
    ) -> httpx.AsyncClient:
        ...
        if trace_http:
            import logging
            logger = logging.getLogger("fluxion.http.trace")
            ...
            async def log_response(res: httpx.Response) -> None:
                await res.aread()
                logger.debug(f"Response: {res.status_code} {res.url}")
    ...
    async def probe(
        self, url: str, extra_headers: dict[str, str] | None = None, trace_http: bool = False
    ) -> ProbeResult:
        ...
        if parsed.scheme == "https":
            try:
                ...
                import logging
                logging.getLogger("fluxion.engine").debug("TLS inspection failed during probe: %s", exc)

        async with self._build_client(extra_headers=extra_headers, trace_http=trace_http) as client:
            max_probe_retries = 3
            ...
            try:
                ...
                import logging
                logging.getLogger("fluxion.engine").debug("DNS resolution failed: %s", exc)
    ...
    async def bench(self, url: str, iterations: int = 10) -> BenchResult:
        ...
        try:
            ...
            import logging
            logging.getLogger("fluxion.engine").debug("Throughput measurement failed: %s", exc)
    ...
    async def fetch(
        ...
        """Execute the download: probe, plan, and dispatch."""
        from fluxion.plugins.manager import PluginManager
        pm = PluginManager()
        parsed = urlparse(request.url)
        
        # Load all enabled plugins dynamically into this manager instance
        # so ProtocolPlugin mappings get populated correctly into __protocols.
        for meta in pm.list_plugins():
            if getattr(meta, "enabled", True): # Use getattr for safety if enabled is missing
                try:
                    pm.load(meta.name)
                except Exception:
                    pass

        # 1. Custom Protocol Delegate Check
        handler = pm.get_protocol_handler(parsed.scheme)
        if handler:
            stats = TransferStats(
                start_time=time.monotonic(),
                protocol=Protocol.HTTP1,
                flux_mode=FluxMode.SINGLE,
            )
            output_name = Path(parsed.path).name or "download"
            output_path = request.output or Path.cwd() / output_name
            t0 = time.monotonic()
            
            try:
                bytes_dl = await handler.download(request.url, output_path, timeout=self._timeout)
                stats.bytes_downloaded = bytes_dl
                elapsed = max(time.monotonic() - t0, 0.001)
                speed = bytes_dl / elapsed
                return FetchResult(
                    url=request.url,
                ...
                raise NetworkError(f"Plugin handler failed: {exc}") from exc

        # 2. Standard HTTP/HTTPS Processing
        if parsed.scheme in ("ftp", "sftp", "scp"):
            return await self._fetch_non_http(request, parsed.scheme)
            
        if parsed.scheme not in ("http", "https"):
            raise NetworkError(f"Unsupported protocol: {parsed.scheme}")

        # Initialize stats for HTTP/HTTPS path
        stats = TransferStats(
            start_time=time.monotonic(),
            protocol=Protocol.HTTP1, # Will be updated by _detect_protocol
            flux_mode=FluxMode.SINGLE, # Will be updated by parallel/single stream logic
        )
        ...
        # Smart Caching: Conditional Requests
        from fluxion.performance.cache import HTTPCache
        cache = HTTPCache()
        if output_path.exists() and output_path.stat().st_size > 0:
            cond_headers = cache.get_conditional_headers(request.url)
            if cond_headers:
                try:
                    async with self._build_client(extra_headers={**stealth_headers, **cond_headers}) as client:
                        cache_resp = await client.head(request.url)
                        if cache_resp.status_code == 304:
                            # 304 Not Modified -> file is fresh
                            stats.phase = TransferPhase.COMPLETE
                            stats.bytes_downloaded = output_path.stat().st_size
                            stats.bytes_total = output_path.stat().st_size
                            ...
                            return FetchResult(
                                ...
                                protocol_used=Protocol.HTTP1 if cache_resp.http_version == "HTTP/1.1" else Protocol.HTTP2,
                                sha256="",
                                resumed=True,
                            )
                except httpx.HTTPError as exc:
                    import logging
                    logging.getLogger("fluxion.engine").debug(f"Cache ETag re-validation failed quietly: {exc}")

        # Phase: probe
        probe = await self.probe(request.url, extra_headers=stealth_headers, trace_http=request.trace_http)
        ...
        if supports_range and total_size and total_size > request.chunk_size * 2 and not request.rate_limit_kbps:
            # Parallel download
        ...
        cache.update_cache(request.url, probe.headers)
    ...
    async def _parallel_download(
        ...
        async with self._build_client(extra_headers=stealth_headers, trace_http=request.trace_http) as client:
                    ...
                    async with client.stream("GET", request.url) as resp:
                        ...
                        with output_path.open("wb") as f:
                            async for chunk in resp.aiter_bytes(chunk_size=65536):
                                ...
                                if request.rate_limit_kbps:
                                    expected_time = len(chunk) / (request.rate_limit_kbps * 1024.0)
                                    sleep_time = expected_time - elapsed
                                    if sleep_time > 0:
                                        await asyncio.sleep(sleep_time)
                                        # Adjust next interval so we don't under-calculate speed due to sleep
                                        last_time = time.monotonic()
