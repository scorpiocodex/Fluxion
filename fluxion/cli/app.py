...
app = typer.Typer(
    name="fluxion",
    help="""
[bold cyan]⟡ FLUXION[/bold cyan] : [dim]The Intelligent Network Command Engine[/dim]

[magenta]Advanced protocol abstraction, quantum-threaded transfers, and deep reconnaissance.[/magenta]
""",
    no_args_is_help=True,
    add_completion=False,
    rich_markup_mode="rich",
    epilog="[dim]Powered by ScorpioCodeX | Quantum-Grade Reliability[/dim]",
)
...
def _make_console() -> Console:
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            import logging
            logging.getLogger("fluxion.cli").debug("Could not enable VT100: %s", exc)
        ...
        if modern:
            ...
            try:
                ...
                import logging
                logging.getLogger("fluxion.cli").debug("Could not reconfigure stdout/stderr: %s", exc)
...
@app.command()
def fetch(
    url: str = typer.Argument(..., help="URL to download (http, https, ipfs, etc.)", show_default=False),
    ...
    rate_limit: Optional[int] = typer.Option(None, "--rate-limit", help="Throttle download speed (KB/s)"),
    trace_http: bool = typer.Option(False, "--trace-http", help="Enable deep HTTP request/response tracing"),
...
def plugin(
    ...
    if action == "list":
        ...
        if json_out:
            ...
            from fluxion.hud.panels import QUANTUM_BLUE

            table = Table(title=f"[{QUANTUM_BLUE}]INSTALLED PLUGINS[/{QUANTUM_BLUE}]", border_style=QUANTUM_BLUE, box=box.HEAVY_EDGE)
...
def _load_plugins() -> None:
    """Dynamically discover and register CLI command plugins before app execution."""
    from fluxion.plugins.manager import PluginManager
    ...
    try:
        manager = PluginManager()
        for p_meta in manager.list_plugins():
            if not p_meta.enabled:
                continue
            try:
                plugin = manager.load(p_meta.name)
                if isinstance(plugin, CommandPlugin):
                    # For CommandPlugins to register correctly on the main Typer instance
                    plugin.register_commands(app)
            except Exception as exc:
                logging.getLogger("fluxion.plugins").warning(f"Failed to load plugin {p_meta.name}: {exc}")
    except Exception:
        pass
